...
class WandernCommand:
    def init(self, interactive: bool = False, directory: str | None = None):
        ...
        if interactive:
            migration_dir = questionary.path("Enter the path to the migration directory:", only_directories=True).ask()
        ...
        self.save_config(config, config_dir)
        rich.print_json(json.dumps(asdict(config)), indent=4)
        if interactive:
            rich.print(f"[bold][green]Initialized wandern config in {config_dir}[/green][/bold]")
        else:
            rich.print("[yellow]Created config with empty dsn, please edit the dsn manually.[/yellow]")
